...
def stackImages(scale, imgArray):
    ...
    if rowsAvailable:
        ...
        ver = hor
    return ver
track_bars = "TrackBars"
cv2.namedWindow(track_bars) # create a new window
...
while True:
    #img = cv2.imread('../Resources/lamborgini.jpg') #for image
    cap = cv2.VideoCapture(0)
    _, img = cap.read()  # for Video
    imgHSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    h_min = cv2.getTrackbarPos("Hue Min", track_bars)
    h_max = cv2.getTrackbarPos("Hue Max", track_bars)
    s_min = cv2.getTrackbarPos("Sat Min", track_bars)
    s_max = cv2.getTrackbarPos("Sat Max", track_bars)
    v_min = cv2.getTrackbarPos("Val Min", track_bars)
    v_max = cv2.getTrackbarPos("Val Max", track_bars)
